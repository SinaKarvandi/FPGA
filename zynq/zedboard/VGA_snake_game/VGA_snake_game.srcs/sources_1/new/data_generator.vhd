...
architecture Behavioral of data_generator is
    ...
    ------------------------------- Snake manager signals --------------------------------

    signal snake_mgr_start_snake      : STD_LOGIC := '0';
    signal snake_mgr_perform_movement : STD_LOGIC := '0';

    signal snake_mgr_new_head_index_row    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0'); -- new head of snake (row)
    signal snake_mgr_new_head_index_column : STD_LOGIC_VECTOR(31 downto 0) := (others => '0'); -- new head of snake (column)
...
begin
    ...
    snk_mgr : snake_manager
        ...
        Port Map(
            i_clk                    => i_clk,
            i_reset_n                => i_reset_n,
            i_start_snake            => snake_mgr_start_snake,
            i_perform_movement       => snake_mgr_perform_movement,
            ...
            o_index_column           => snake_mgr_index_column, -- index to array (column)
            o_color                  => snake_mgr_color
        );
                ...
                case state is
                    ...
                    when IN_THE_MIDDLE_OF_INITIALIZATION =>
                        ...
                        if is_initialize_completed = '1' then
                            state                 <= INITIALIZING_SNAKE;
                            snake_mgr_start_snake <= '1';
                        end if;

                    when INITIALIZING_SNAKE =>
